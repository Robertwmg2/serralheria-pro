...
const ClientesPage = () => {
  ...
  function abrirEditar(c: Cliente) {
    setEditando(c);
    setForm({ nome: c.nome, telefone: c.telefone, email: c.email || "", endereco: c.endereco || "", obs: c.obs || "" });
    setOpen(true);
  }
  function salvar() {
                ...
                <div className="flex justify-between items-start gap-2">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="space-y-1 mt-2 text-sm text-muted-foreground">
                      ...
                      {c.endereco && <div className="flex items-center gap-2"><MapPin className="h-3.5 w-3.5" />{c.endereco}</div>}
                    </div>
                  </div>
                </div>
                <div className="flex gap-1 mt-3 pt-3 border-t border-border">
                  <Button size="sm" variant="ghost" onClick={() => enviarWhatsApp(c.telefone, `Olá ${c.nome},`)}>
                    <MessageCircle className="h-4 w-4 text-success" />
                  </Button>
                  <Button size="sm" variant="ghost" onClick={() => abrirEditar(c)}>
                    <Pencil className="h-4 w-4" />
                  </Button>
                  <Button size="sm" variant="ghost" onClick={() => { removeCliente(c.id); toast.success("Removido"); }}>
