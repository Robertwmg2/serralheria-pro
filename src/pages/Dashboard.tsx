...
import { useStore, formatBRL, calcularOrcamento } from "@/lib/store";
import { Users, FileText, Boxes, Calendar, TrendingUp, Flame, Wrench, AlertTriangle } from "lucide-react";
import { Link } from "react-router-dom";
import { differenceInDays } from "date-fns";

const Dashboard = () => {
  const { clientes, orcamentos, materiais, ferramentas, servicos, lancamentos } = useStore();
  ...
  const stats = [
    { label: "Clientes", value: clientes.length, icon: Users, link: "/clientes", color: "text-primary" },
    { label: "Orçamentos", value: orcamentos.length, icon: FileText, link: "/orcamentos", color: "text-accent" },
    { label: "Materiais", value: materiais.length, icon: Boxes, link: "/materiais", color: "text-success" },
    { label: "Agendados", value: proxServicos.length, icon: Calendar, link: "/agenda", color: "text-primary-glow" },
  ];
      ...
      <div className="space-y-6">
          ...
          <div className="relative flex items-center gap-4">
            ...
            <div>
              <h2 className="font-display text-2xl md:text-3xl font-bold">
                Bem-vindo de volta à <span className="text-ember">oficina</span>
              </h2>
              <p className="text-muted-foreground text-sm mt-1">
                Tudo que sua serralheria precisa em um só lugar — orçamentos, agenda, financeiro e IA.
              </p>
            </div>
          </div>
        </Card>
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <Card className="panel p-5">
            <div className="text-xs uppercase tracking-wider text-muted-foreground">Total Orçado</div>
            <div className="text-2xl font-display font-bold mt-1">{formatBRL(totalOrcado)}</div>
            <div className="text-xs text-muted-foreground mt-1">{orcamentos.length} orçamentos</div>
          </Card>
          <Card className="panel p-5 border-success/30">
            <div className="text-xs uppercase tracking-wider text-success">Aprovados</div>
            <div className="text-2xl font-display font-bold mt-1">{formatBRL(receita)}</div>
            <div className="text-xs text-muted-foreground mt-1">{aprovados.length} aprovados</div>
          </Card>
          <Card className="panel p-5">
            <div className="text-xs uppercase tracking-wider text-muted-foreground">Caixa</div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
              ...
              <ul className="space-y-2 text-sm">
                {ferramentasAlerta.map((f) => (
                  <li key={f.id} className="flex items-center gap-2">
                    ...
                    Manutenção: <strong>{f.nome}</strong> — {new Date(f.proximaManutencao).toLocaleDateString("pt-BR")}
                  </li>
                ))}
                {materiaisVencendo.map((m) => (
                  <li key={m.id} className="flex items-center gap-2">
                    <Boxes className="h-3.5 w-3.5 text-warning" />
                    Validade: <strong>{m.nome}</strong> — {new Date(m.validade!).toLocaleDateString("pt-BR")}
                  </li>
                ))}
              </ul>
            )}
          ...
          <Card className="panel p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-display font-semibold">Próximos serviços</h3>
            </div>
